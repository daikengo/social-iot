class FeedController < ApplicationController
    ...
    #@feed=Feed.where("channel_id = ?",@channel.id).select("?",feed_select_data(@channel)).first
		@success = channel_permission?(@channel, @api_key)
    ...
    if params[:round]
      @feed = item_round(@feed, params[:round].to_i)
        ...
        #puts @feed["field#{params[:field_id]}"]
				output = @feed["field#{params[:field_id]}"]
			else
				output = @feed.to_json
			end
		# else set error code
	...
	private
      ...
      only += [:field9] unless channel.field9.blank?
      only += [:field10] unless channel.field10.blank?
      only += [:field11] unless channel.field11.blank?
      only += [:field12] unless channel.field12.blank?
      only += [:field13] unless channel.field13.blank?
      only += [:field14] unless channel.field14.blank?
      only += [:field15] unless channel.field15.blank?
      only += [:field16] unless channel.field16.blank?
      only += [:gps_precision] unless (channel.field6.blank? and channel.field7.blank?)
      ...
      only += [:field9] unless channel.field9.blank? or (params[:field_id] and params[:field_id] != '9')
      only += [:field10] unless channel.field10.blank? or (params[:field_id] and params[:field_id] != '10')
      only += [:field11] unless channel.field11.blank? or (params[:field_id] and params[:field_id] != '11')
      only += [:field12] unless channel.field12.blank? or (params[:field_id] and params[:field_id] != '12')
      only += [:field13] unless channel.field13.blank? or (params[:field_id] and params[:field_id] != '13')
      only += [:field14] unless channel.field14.blank? or (params[:field_id] and params[:field_id] != '14')
      only += [:field15] unless channel.field15.blank? or (params[:field_id] and params[:field_id] != '15')
      only += [:field16] unless channel.field16.blank? or (params[:field_id] and params[:field_id] != '16')
      only += [:gps_precision] unless channel.field6.blank? and channel.field7.blank?
